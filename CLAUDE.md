...
- **`apa_citations.py`** (root) — rewrites the answer LLM's numeric citations into **APA7**: `/query` calls LightRAG's `aquery_llm` (returns the answer **and** the `reference_id → file_path` map in one call — so no `LightRAG/` patch), then this module replaces in-text `[n]` with `(Author, Year)` and rebuilds `### References` from a filename→bib-record manifest (`papers_metadata.json`, built by [scripts/build_apa_manifest.py](scripts/build_apa_manifest.py) via Crossref + LLM extraction). `/query` returns `{answer, references, mode}`. Deploy `apa_citations.py` + `papers_metadata.json` next to `query_server.py`. Server env: `APA_MANIFEST`, `HADES_PAPERS_BASE`.
- **`aprag/`** — an installable package (`pip install -e .`) exposing two thin clients over `query_server.py`: the **`aprag`** CLI (`aprag ask` = synthesized, `aprag chunks` = raw chunks) and the **`aprag-mcp`** stdio MCP server (tools **`aprag_query`** + **`aprag_retrieve`**; modes `hybrid`/`local`/`global`/`mix`/`naive`). Server URL via `APRAG_QUERY_URL`. `aprag/references.py` does **client-side** APA citation linking: it matches each cited PDF by filename under `$APRAG_PAPERS_DIR` and rewrites the references to clickable `file://` links, falling back to the hades share path (`hades.psych.ualberta.ca:/Users/Shared/aprag_papers`). Client machines run no models and hold no data. Full deployment + access instructions: [APRAG_ACCESS.md](APRAG_ACCESS.md).
...
- **Embedding dim is 4096** (Octen-Embedding-8B), not the 3072 of the old `text-embedding-3-large` path. `docs/PIPELINE_README.md` describes the **superseded** Qwen2.5-72B + OpenAI-embeddings + NanoVectorDB setup — treat the code, `APRAG_ACCESS.md`, and `SESSION_HANDOFF.md` as current when they disagree.
...
`docs/` contains operational notes worth consulting before large runs: `PRE_FLIGHT.md`, `BOTTLENECKS.md`, `JOB_STATUS_CHECK.md`, `PARALLEL_DOCS_FIX.md`, `UNKNOWN_SOURCE_233_DOCS.md` (alongside `CANONICAL_INGEST_PARAMS.md`, `CHUNKER_AND_PIPELINE_NOTES.md`, `SESSION_HANDOFF.md`, `PC_OCTEN_SETUP.md`), plus the top-level `LIGHTRAG_HOW_IT_WORKS.md`. These are point-in-time and may be stale — verify against the code.
