"""
query.py — Interactive QA over academic papers using Claude Sonnet 4.5.

Uses the knowledge graph built by ingest.py.
Requires ANTHROPIC_API_KEY and OPENAI_API_KEY (for embeddings) to be set.

Usage:
    cd /Users/devon7y/VS_Code/rag_testing/LightRAG
    source .venv/bin/activate
    python ../query.py
    python ../query.py --mode global   # broad cross-paper questions
    python ../query.py --mode local    # specific paper/entity questions

Query modes:
    hybrid  — combines local + global  (default, best for most questions)
    ...
    mix     — KG + vector search combined (best with a reranker)
...
async def claude_llm(prompt, system_prompt=None, history_messages=None, **kwargs):
    """LLM function using Claude Sonnet 4.5 via Anthropic API."""
...
async def main(mode: str):
    ...
    if not STORAGE_DIR.exists():
        ...
        print("Run ingest.py first to build the index from your papers.")
        sys.exit(1)

    print(f"Academic Papers RAG  |  model: {CLAUDE_MODEL}  |  mode: {mode}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Query academic papers RAG with Claude")
    parser.add_argument(
        "--mode",
        default="hybrid",
