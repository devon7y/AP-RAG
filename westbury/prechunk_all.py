#!/usr/bin/env python3
"""Pre-chunk all PDFs through the scientific chunker and save to a JSON cache.

This avoids CPU-bound chunking during the main ingest pipeline, allowing
PARALLEL_DOCS > 1 without blocking the asyncio event loop.

Usage:
    WORKDIR=/scratch/devon7y/westbury_rag python prechunk_all.py

Outputs:
    $WORKDIR/$STORAGE_SUBDIR/chunk_cache.json
...
def _process_pdf(index: int, pdf_path: Path, tokenizer, config: ChunkerConfig) -> dict:
    try:
        ...
        # Hash content identically to ingest script:
        #   doc_id = "doc-" + hashlib.md5(text.encode("utf-8")).hexdigest()
        # LightRAG passes this same text to chunking_func.
...
def main():
    # Validate tokenizer availability once up-front (and warm cache).
    ...
    print("MAX_DOC_TOKENS: unused for prechunk (full document chunking)\n", flush=True)
    print(
        "Workers: "
        f"{workers} "
