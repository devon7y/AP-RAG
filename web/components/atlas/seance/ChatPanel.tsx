...
function Message({ msg }: { msg: SeanceMessage }) {
  ...
  return (
    <div
      ...
      style={{ borderColor: msg.silent ? "var(--atlas-baseline)" : GHOST_GREEN }}
    >
          ...
          <div className="space-y-2 pt-2">
            {STARTERS.map((q) => (
              <button
                key={q}
                onClick={() => onAsk(q)}
                className="block w-full rounded-lg border border-hairline px-3 py-2 text-left text-sm text-ink-2 transition-colors hover:border-[color:var(--atlas-accent)] hover:text-ink"
              >
                {q}
              </button>
            ))}
      ...
      <footer className="border-t border-hairline p-3">
        <div className="flex items-end gap-2">
          <textarea
            ...
            disabled={channeling}
            className="hud-scroll min-h-[3.25rem] flex-1 resize-none rounded-lg border border-hairline bg-page/60 px-3 py-2 text-sm text-ink outline-none placeholder:text-ink-3 focus:border-[color:var(--atlas-accent)] disabled:opacity-50"
          />
          <button
            onClick={send}
