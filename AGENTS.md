...
- **`LightRAG/`** — upstream LightRAG, its own git repo and its own `LightRAG/AGENTS.md` (consult that file for library internals: the `LightRAG` class, storage backends, query modes, `operate.py`). It is `pip install -e`'d as `lightrag_hku` into both `venv/` (Python 3.14) and `LightRAG/.venv/`. **Do not edit `LightRAG/` unless you are deliberately patching the upstream library**; AP-RAG behavior is driven by passing custom functions (e.g. `chunking_func=`) into the stock `LightRAG` class, not by forking it.
